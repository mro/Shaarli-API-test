...
type auth = { uid : uid; pwd : pwd }

type par = { name : name; value : value }
...
module P = struct
  ...
  let scheme' =
    let to_ s = Scheme s and of_ (Scheme o) = o in
    conv to_ of_ (pcre "https?")

  let uid' =
    let to_ s = Uid s and of_ (Uid o) = o in
    conv to_ of_ (pcre "[^:]*")

  let pwd' =
    let to_ s = Pwd s and of_ (Pwd o) = o in
    conv to_ of_ (pcre "[^@]*")

  let auth' =
    let to_ (uid, pwd) = { uid; pwd } and of_ { uid; pwd } = (uid, pwd) in
    conv to_ of_ (uid' <* char ':' <&> pwd' <* char '@')

  let host' =
    let to_ s = Host s and of_ (Host o) = o in
    conv to_ of_ (pcre "[^:/&?]*")

  let port' =
    let to_ s = Port s and of_ (Port o) = o in
    conv to_ of_ int
  ...
  let name' =
    let to_ s = Name s and of_ (Name o) = o in
    conv to_ of_ (pcre "[^=&]+")

  let value' =
    let to_ s = Value s and of_ (Value o) = o in
    conv to_ of_ (pcre "[^&]*")

  let par' =
    let to_ (name, value) = { name; value }
    and of_ { name; value } = (name, value) in
  ...
  let full' =
    ...
    conv to_ of_
      ( scheme' <* char ':' <* str "//"
      <&> (opt auth' <&> (host' <&> opt (char ':' *> port')))
      <&> (path' <&> query') <* stop )

  let full'' = compile full'
end

let parse str = Tyre.exec P.full'' str
