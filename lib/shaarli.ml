...
let sift_title soup =
  match
    soup
    |> select_one
         "html > body form[name='configform'] input[name='title'][type='text']"
  with
  | None -> None
...
let sift_linkform soup =
  ...
  |> fold
       (fun li no ->
         match attribute "name" no with
         | None -> li (* cannot happen, see CSS selector *)
         | Some na -> (
             match name no with
             | "textarea" ->
                 let va = no |> texts |> String.concat " " in
                 li |> List.cons (na, va)
             | "input" -> (
                 match attribute "type" no with
                 | Some "checkbox" ->
                     let va =
                       match attribute "checked" no with
                       ...
                       | Some "off" | None -> checkbox_unchecked
                       | _ -> checkbox_checked
                     in
                     li |> List.cons (na, va)
                 | Some "hidden" | Some "text" ->
                     let va =
                       match attribute "value" no with
                       | None -> ""
                       | Some v -> v
                     in
                     li |> List.cons (na, va)
