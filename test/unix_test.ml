...
let submit_request ~address ~port ~kind ~path ~referer ~user_agent =
  ...
  let request =
    ...
    ^ ( match user_agent with
      | None -> ""
      | Some ua -> Printf.sprintf "User-Agent: %s\r\n" ua )
    ^ ( match referer with
      | None -> ""
      | Some referer -> Printf.sprintf "Referer: %s\r\n" referer )
    ^ ( match post_data with
      | None -> ""
      | Some post_data ->
          let post_data = serialize ~post_data in
          "Content-type: application/x-www-form-urlencoded\r\n"
          ^ "Content-length: "
          ^ string_of_int (String.length post_data)
          ^ "\r\n" ^ "Connection: close\r\n" ^ "\r\n" ^ post_data )
    ^ "\r\n"
  in
...
let test_http_get () =
  let inchan, _ =
    submit_request ~address:"l.mro.name" ~port:80 ~kind:GET ~path:"/"
