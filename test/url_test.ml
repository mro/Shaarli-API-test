...
let test_full () =
  let raw =
    "https://uid:pwd@example.com:123/a/b.c/d.e?foo=bar&bar=baz&foo=bar"
  in
  ...
  assert (url.path = List.map (function s -> Dir s) [ "/a"; "/b.c"; "/d.e" ]);
  assert (
    url.query
    = List.map
        (function k, v -> { name = Name k; value = Value v })
...
let test_noquery () =
  ...
  assert (Host "example.com" = url.host);
  assert (
    url.path = ([ "/a"; "/b.c"; "/d.e" ] |> List.map (function s -> Dir s)) );
  assert ([] = url.query)
