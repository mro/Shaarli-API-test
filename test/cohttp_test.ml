...
let do_get dst =
  ...
  let op (resp, _) =
    resp |> Response.status |> Cohttp.Code.code_of_status |> string_of_int
    |> return
...
let timeout_promise timeout promise =
  Lwt.pick
    [ promise; (Lwt_unix.sleep timeout >>= fun () -> "Timeout" |> return) ]

let test_get () =
  let ret =
    "https://mro.name/pin4sha" |> Uri.of_string |> do_get |> Lwt_main.run
  in
  assert ("302" = ret)

let test_get_timeout () =
  let ret =
    "https://mro.name/pin4sha" |> Uri.of_string |> do_get
    |> timeout_promise 0.001 |> Lwt_main.run
  in
  assert ("Timeout" = ret)
...
let v1_posts_get_param_url_to_post uri =
  ...
  | Some url ->
      ...
      let ret =
        (u2 |> Uri.path) ^ "/../../../../"
        |> Uri.with_path u2 |> Uri.canonicalize
      in
      Ok ret
...
let test_v1_posts_get_param_url_to_post () =
  match
    ...
    |> Uri.of_string |> v1_posts_get_param_url_to_post
  with
  ...
  | Ok u ->
      ...
      assert (
        Some "http://sebsauvage.net/wiki/doku.php?id=php:shaarli"
        = Uri.get_query_param u "post" );
      assert (
        "https://demo.0x4c.de/shaarli-v0.41b/?post=http://sebsauvage.net/wiki/doku.php?id=php:shaarli"
        = Uri.to_string u )
 ...
 *)
let shaarli_get _ = Ok [ ("key", "value") ]
...
let test_get_shaarli () =
  ...
  let ret =
    "https://demo.0x4c.de/shaarli-v0.41b?post=http%3A%2F%2Fsebsauvage.net%2Fwiki%2Fdoku.php%3Fid%3Dphp%3Ashaarli"
    |> Uri.of_string |> do_get |> timeout_promise 4. |> Lwt_main.run
  in
  assert ("Timeout" <> ret)
