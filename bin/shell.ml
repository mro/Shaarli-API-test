...
let pin_posts_get_url _ u = Error [ "not implemented yet"; u ]
...
let exec_url url =
  ...
  | Dir "/posts" -> (
      match List.hd htap with
      | Dir "/get" ->
          exec_posts_get
            { url with path = tl |> List.tl |> List.rev; query = [] }
            url.query
...
let exec args =
  ...
  | arg -> (
      ...
      | "--doap" ->
          ...
          0
      | url -> (
          match url |> Lib.Url.parse with
          | Error _ -> err 3 [ "Couldn't parse url" ]
          | Ok url' -> (
              match url' |> exec_url with Error e -> err 4 e | Ok c -> c ) ) )
